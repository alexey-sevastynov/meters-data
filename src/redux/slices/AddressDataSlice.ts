...
export const fetchAllAddressData = createAsyncThunk<
  AddressDataType[],
  void,
  { rejectValue: AxiosError }
>(actionNames.addressData.fetchAll, async (_, { rejectWithValue }) => {
  try {
    const { data } = await axios.get<AddressDataType[]>(
      `${API_URL}${API_PATH.utilityAccounts}`
    );
    return data;
  } catch (error: unknown) {
